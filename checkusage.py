...
class CheckUsage(object):
    def __init__(self, limit = 1000,
        ...
        database, cursor = self.connect_mysql( 's' + str(mysql_default_server) + '.labsdb')
        self.servers[mysql_default_server] = (database, cursor)

        # Find where the databases are located
        cursor.execute('SELECT CONCAT( dbname, "_p" ) AS dbname, REPLACE( url, "http://", "" ) AS domain, slice AS server FROM meta_p.wiki ORDER BY size DESC LIMIT %s', (limit, ))
        for dbname, domain, server in cursor.fetchall():
            if server not in self.servers:
                self.servers[server] = self.connect_mysql(str(server))
